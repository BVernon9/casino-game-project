...
def main_menu():
    ...
    while running:
        ...
        menu_text = font.render("1 for Blackjack, 2 for Roulette, 3 for Slots, 4 for Horse Racing, ESC to Exit", True, WHITE)
...
def handle_key_events():
    global running  # Ensure that 'running' is accessible and modifiable here
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
            ...
            if event.key == pygame.K_ESCAPE:
                running = False
                main_menu()  # Set running to False to exit the loop
            # Existing game control keys
...
symbols_images = {
    name: pygame.transform.scale(pygame.image.load(f"C:/Users/darre/OneDrive/Documents/Work/Computational/PNGS/{name}.png"), SYMBOL_SIZE)
...
def slots_game():
    global player_balance, last_spin_symbols, outcome_message
    running = True
    spin_active = False
